...
namespace WebUI.AgentHost.Utilities;

public static class ChatClientExtensions
{
    public static async Task<ChatClientBuilder> AddChatClientAsync(this IHostApplicationBuilder builder, string connectionName)
    {
        ...
        var chatClientBuilder = connectionInfo.Provider switch
        {
            ...
            ClientChatProvider.FoundryLocal => await builder.AddOpenAIFoundryLocalClientAsync(connectionName, connectionInfo),
    ...
    private static async Task<ChatClientBuilder> AddOpenAIFoundryLocalClientAsync(this IHostApplicationBuilder builder, string connectionName, ChatClientConnectionInfo connectionInfo)
    {
    ...
    public static async Task<ChatClientBuilder>  AddKeyedChatClientAsync(this IHostApplicationBuilder builder, string connectionName)
    {
        var cs = builder.Configuration.GetConnectionString(connectionName);
        ...
        var chatClientBuilder = connectionInfo.Provider switch
        {
            ...
            ClientChatProvider.FoundryLocal => await builder.AddKeyedOpenAIFoundryLocalClientAsync(connectionName, connectionInfo),
            _ => throw new NotSupportedException($"Unsupported provider: {connectionInfo.Provider}")
        };
